...
def create_slicer_fcsv(
    ...
    for ii in range(pts_mat.shape[0]):
        outlines.append(
            ...
            + ","
            + str(pts_mat[ii, 2])
            + f",{pt_orientation[0]},{pt_orientation[1]},"
            + f"{pt_orientation[2]},{pt_orientation[3]},"
...
def projected_onto_line(points, line_N, line_P):
    """
    Projects a set of points onto a line
    defined by a direction vector and a point on the line.

    Args:
        points (np.ndarray): An array of points to be projected.
                             Shape (n_points, n_dimensions).
        line_N (np.ndarray): A direction vector of the line.
                             Shape (n_dimensions,).
    ...
    Note:
        This function normalizes the direction
        vector `line_N` before performing the projection.
...
def get_highest_level_info(filepath, return_order="xyz"):
    ...
    zarr_axis_order = [
        field["name"] for field in metadata["multiscales"][0]["axes"]
    ]
...
def get_additional_channel_image_at_highest_level(
    image_path,
    ants_template,
    input_orientations,
