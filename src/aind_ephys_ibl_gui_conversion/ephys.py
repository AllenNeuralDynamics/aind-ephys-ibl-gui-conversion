...
def remove_overlapping_channels(recordings) -> list[si.BaseRecording]:
    ...
    for index, recording in enumerate(recordings):
        ...
        for index in remove_indices:
            channel_id_remove = [channel_id for channel_id in recording.channel_ids if str(index + 1) in channel_id][0]
...
def extract_continuous(sorting_folder: Path,results_folder: Path, min_duration_secs: int = 300,
    ...
    for stream_name, main_recordings_streams in main_recordings.items():
        ...
        max_samples_lfp = max([recording.get_num_samples() for recording in main_recordings[stream_name]])
        main_recording_lfp = spre.highpass_filter([recording for recording in main_recordings[stream_name] if recording.get_num_samples() == max_samples_lfp][0])

        max_samples_ap = max([recording.get_num_samples() for recording in main_recordings[stream_name.replace('LFP', 'AP')]])
        main_recording_ap = spre.highpass_filter([recording for recording in main_recordings[stream_name.replace('LFP', 'AP')] if recording.get_num_samples() == max_samples_ap][0])
