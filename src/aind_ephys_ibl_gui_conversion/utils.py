...
def fscale(ns, si=1, one_sided=False):
    """
    numpy.fft.fftfreq returns Nyquist as a
    negative frequency so we propose this instead
...
class WindowGenerator(object):
    ...
    Provide sliding windows indices generator for
    signal processing applications.
    For straightforward spectrogram / periodogram implementation,
    prefer scipy methods !
    ...
    def __init__(self, ns, nswin, overlap):
        """
        :param ns: number of sample of the signal along the
                   direction to be windowed
    ...
    def slice_array(self, sig, axis=-1):
        ...
        slices corresponding to windows.
        Especially useful when working on memmpaps
    ...
    def tscale(self, fs):
        """
        Returns the time scale associated with
        Window slicing (middle of window)
